...
// type PenColor = "#FF0000" | "#00FF00" | "#0000FF" | "#000000";
// type PenWidth = 2 | 4 | 6 | 8;
...
const ScreenRecorder = () => {
  ...
  const [selectedColor, setSelectedColor] = useState<PenColor>("#000000");
  const [selectedWidth, setSelectedWidth] = useState<PenWidth>(2);
  ...
  const startRecordingSource = async (sourceId?: string) => {
    try {
      ...
      ////////////////////////////// Drawing overlay
      // if (enableDrawing) {
      await (window as any).electron.createOverlayWindow();
      // Start with click-through enabled
      await (window as any).electron.setClickThrough(true);
      // }
      //////////////////////////////
  ...
  const stopRecording = async () => {
    ...
    try {
      ...
      // Close drawing overlay if it's enabled
      // Close overlay window when recording stops
      await (window as any).electron.closeOverlayWindow();
      ...
      <div className="max-w-4xl mx-auto">
        {!showPicker && (
          <div className="bg-white rounded-lg shadow-lg p-6">
              ...
              <div className="mb-6 space-y-4">
                <div className="flex items-center gap-2">
                  <input
                    ...
                    // onChange={async (e) => {
                    //   setEnableDrawing(e.target.checked);
                    //   if (e.target.checked) {
                    //     console.log("Enabling drawing overlay");
                    //     await (window as any).electron.createOverlayWindow();
                    //     await (window as any).electron.setClickThrough(false);
                    //   } else {
                    //     console.log("Disabling drawing overlay");
                    //     await (window as any).electron.closeOverlayWindow();
                    //     await (window as any).electron.setClickThrough(true);
                    //   }
                    // }}
                    onChange={async (e) => {
                      setEnableDrawing(e.target.checked);
                      await (window as any).electron.setClickThrough(
                        !e.target.checked
                      );
                    }}
                    className="w-4 h-4 text-blue-600 rounded border-gray-300 focus:ring-blue-500"
            ...
            {/*  add this after your recording button: */}
            <VideoLoader />
            {/* End Video Loader */}
            ...
            {isRecording && (
              <DrawingTools
                ...
                onSelectTool={async (tool) => {
                  ...
                  await (window as any).electron.setDrawingTool({
                    tool,
                    color: selectedColor,
                    width: selectedWidth,
                  });
